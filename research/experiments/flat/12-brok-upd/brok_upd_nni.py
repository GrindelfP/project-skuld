...
def train(brok: BrokNet,
          ...
          lr: float = 5e-4,
          mode: str = "joint",  # Options: "joint" or "individual"
    ...
    # Initialize optimizer for all parameters in BrokNet
    optimizer = torch.optim.Adam(brok.parameters(), lr=lr)

    # Use Cosine Annealing to decay learning rate from lr to lr/10 over n_epochs
    ...
    for epoch in range(1, n_epochs + 1):
        # We will track individual losses to display in the log
        current_expert_losses = []
        epoch_loss_val = 0.0

        if mode == "joint":
            # --- JOINT TRAINING MODE ---
            # Standard approach: compute all losses, average them, then update
            optimizer.zero_grad()
            expert_losses_tensors = []

            for i, (a, b, m, n) in enumerate(param_sets):
                # Generate synthetic batch for the specific integrand variant
                u, f_tilde = make_expert_batch(a, b, m, n, n_per_expert, device, norm_cache)

                # Compute 3rd order mixed partial derivative for the i-th expert
                dG = mixed_partial_3_expert(brok.experts[i], u)

                # Calculate MSE loss for this expert
                loss_i = loss_fn(dG, f_tilde)
                expert_losses_tensors.append(loss_i)
                current_expert_losses.append(loss_i.detach())

            # Aggregate losses and perform a single optimization step
            total_loss = torch.stack(expert_losses_tensors).mean()
            total_loss.backward()

            # Gradient clipping to prevent instability in SIREN
            ...
            # Specialized approach: update each expert independently within the epoch
            temp_losses = []
            for i, (a, b, m, n) in enumerate(param_sets):
                # Clear gradients before processing the current expert
                optimizer.zero_grad()
                ...
                # Backward pass only for this specific expert's loss
                loss_i.backward()

                # Clip gradients for the specific expert sub-network
                torch.nn.utils.clip_grad_norm_(brok.experts[i].parameters(), max_norm=5.0)

                # Update weights for this expert immediately
                optimizer.step()

                detached_loss = loss_i.detach()
                temp_losses.append(detached_loss)
                current_expert_losses.append(detached_loss)

            # Calculate mean loss of the epoch for history tracking
        ...
        if epoch % verbose_every == 0 or epoch == 1:
            ...
            # Format individual expert losses for display
            per_expert_str = "  ".join(
                f"S{i}={el.item():.2e}"
    ...
    # Final summary after training completion
...
def main():
    ...
    # ── Hyperparameters ───────────────────────────────────────────────────
    HIDDEN       = [64, 128, 64]    # hidden layer widths per SindriNet expert
    OMEGA_0      = 30.0
    N_EPOCHS     = 8000
    N_PER_EXPERT = 8192 # 1024            # random points per expert per epoch
    LR           = 5e-4
