...
def integrand_xyz(x, y, u, a, b, m, n):
    alp1 = x
    alp2 = y * (1.0 - x)
    eps = 1e-9
    u_c = np.clip(u, eps, 1.0 - eps)

    R2 = alp1 ** 2 * p2p2 + alp2 ** 2 * p1p1 - (alp1 * alp2) * (PP - p1p1 - p2p2)
    D = (alp1 * alp2) * PP + p1p1 * alp2 * (1 - alp1 - alp2) + p2p2 * alp1 * (1 - alp1 - alp2) + \
        alp1 * m1 ** 2 + alp2 * m2 ** 2 + (1 - alp1 - alp2) * m3 ** 2

    z0 = (u_c / (1.0 - u_c)) * D + u_c * R2
    weight = (u_c ** m) * ((1.0 - u_c) ** (n - m - 2))

    return alp1 ** a * alp2 ** b * (1 - x) * weight * np.exp(-np.clip(z0, -50, 50))


def numerical_reference(a, b, m, n):
    def it(u, x, y): return integrand_xyz(x, y, u, a, b, m, n)

    def iy(y, x): return integrate.quad(it, 0, 1, args=(x, y), limit=200)[0]
...
def build_dataset(n_samples_per_config: int):
    ...
    for a, b, m, n in zip(A_VALS, B_VALS, M_VALS, N_VALS):
        def wrapper(X: torch.Tensor, _a=a, _b=b, _m=m, _n=n):
            xv, yv, uv = X[:, 0].numpy(), X[:, 1].numpy(), X[:, 2].numpy()
            vals = torch.tensor(integrand_xyz(xv, yv, uv, _a, _b, _m, _n), dtype=torch.float32)
            return vals.view(-1, 1)

        X_xyu, y_f = generate_data(
            func=wrapper, lower=[0.0, 0.0, 0.0], upper=[1.0, 1.0, 1.0],
            n_samples=n_samples_per_config, n_dim=3, dis_type="std",
        )
        params_col = torch.tensor([[float(a), float(b), float(m), float(n)]] * X_xyu.shape[0], dtype=torch.float32)
        X_all.append(torch.cat([X_xyu, params_col], dim=1))
        y_all.append(y_f)
...
if __name__ == "__main__":
    ...
    model.fit(x_tr, y_tr, epochs=lbfgs_epochs, verbose=True)

    print("\n" + "=" * 70)
    print(f"{'Config (a,b,m,n)':<20} | {'Scipy Ref':<12} | {'NNI Result':<12} | {'Rel. Error'}")
    print("-" * 70)
