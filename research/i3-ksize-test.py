...
def main() -> None:
    ...
    sample_sizes_k = np.arange(10, 101, 10)
    ...
    for idx, (errors, (_, _, name)) in enumerate(zip(all_errors, genz_functions)):
        ax = axes[idx]
        indices = np.arange(len(sample_sizes_k))
        x_labels = [f"{k}³" for k in sample_sizes_k]
        bars = ax.bar(indices, errors, color='skyblue', edgecolor='navy', alpha=0.8, width=0.6)
        ax.set_yscale('log')
        ax.set_xlabel('k (grid = k³)')
        ax.set_ylabel('Абсолютная ошибка (логарифмическая шкала ошибки)')
        ax.set_title(name)
        ...
        for i, (bar, err) in enumerate(zip(bars, errors)):
            if err > 0:
                ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height(), f'{err:.1e}', ha='center', va='bottom',
                        fontsize=8, rotation=30)

    plt.suptitle('Точность на 3D-функциях Генца\n'
                 '(логарифмическая шкала ошибки)', fontsize=14)
    plt.tight_layout()
    plt.show()
