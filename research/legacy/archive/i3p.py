...
def nni_for(a, b, m, n):
    # scale the parameter values with the same scaler used on X_init
    # columns 3..6 of X_init hold a, b, m, n values
    ...
    nni_scaled = NeuralNumericalIntegration.integrate(
        ...
        theta=theta_scaled,
        unit_cube=True,      # integration over [0,1]^3 (x,y scaled already)
        alphas=[],
        betas=[]
    )
...
for a, b, m, n in zip(A_vals, B_vals, M_vals, N_vals):
    nni = nni_for(a, b, m, n)
    num = numerical_for(a, b, m, n)
    print(f"I({a},{b},{m},{n})  NNI={nni:.8e}  NUM={num:.8e}  err={abs(nni-num):.3e}")
